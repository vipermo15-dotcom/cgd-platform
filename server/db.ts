...
/**
 * 최종합격 시 취업 축하 배너를 자동 생성한다.
 * - 같은 학생+회사 자동 배너가 이미 있으면 중복 생성하지 않는다(상태 재설정 대비).
 * - 통계 정합성을 위해 학생 프로필의 취업상태도 '취업확정'으로 함께 갱신한다.
 * 이미 배너가 있으면 false, 새로 만들면 true를 반환한다.
 */
export async function autoCreateEmploymentBanner(params: {
  ...
  const existing = await db
    .select({ id: employmentSuccessBanner.id })
    .from(employmentSuccessBanner)
    .where(and(
      eq(employmentSuccessBanner.studentUserId, params.studentUserId),
      eq(employmentSuccessBanner.companyName, params.companyName),
      eq(employmentSuccessBanner.isAutoGenerated, true),
    ))
    .limit(1);
  if (existing.length > 0) return false;

  await db.insert(employmentSuccessBanner).values({
    studentUserId: params.studentUserId,
    studentName: params.studentName,
    companyName: params.companyName,
    jobTitle: params.jobTitle,
    useInitial: true,            // 개인정보 보호: 기본 이니셜 처리
    isAutoGenerated: true,
    isActive: true,
  });

  // 취업 통계(/results, 대시보드)와 일관되도록 프로필 취업상태 갱신
  await db
    .update(studentProfiles)
    .set({
      employmentStatus: "취업확정",
      employedCompany: params.companyName,
      employedAt: new Date(),
      updatedAt: new Date(),
    })
    .where(eq(studentProfiles.userId, params.studentUserId));
